...
difference_plot <- difference_summary |>
    ...
    geom_text(aes(label = format(mean, nsmall = 1, digits = 1)), 
              color = "white", family = "bold") +
    scale_x_time(name = "", labels = ~format_hms(.x, s = FALSE),
                 breaks = 12 * 60 * (1:5), minor_breaks = NULL, limits = 60 * c(10, 62)) +
    scale_y_reverse(name = "Standings", breaks = 1:6, limits = c(6.2, 0.5), labels = format_standings) +
    scale_color_gradient(low = scale_most[2], high = scale_most[5]) +
    scale_size(range = c(13, 19)) +
    ggtitle("Average Margin of Elimination", "Advancement difference between eliminated player and survivors") +
    theme_most() +
    theme(
        aspect.ratio = 1,
